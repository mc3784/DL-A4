...
function fp(state)
    ...
    for i = 1, params.seq_length do
        ...
        local s = model.s[i - 1]
        model.err[i], model.s[i],preds = unpack(model.rnns[i]:forward({x, y, s}))
    local val,ind = torch.max(preds,2)
    --print("some preds:")
    --print_preds(ind)
...
function bp(state)
    ...
    for i = params.seq_length, 1, -1 do
        ...
        local tmp = model.rnns[i]:backward({x, y, s},
                                           {derr, model.ds,torch.zeros(preds:size())})[3]
        -- remember (to, from)
...
function run_valid()
    ...
    perplexityToSave = g_f3(torch.exp(perp / len))
    -- save model
    --model:get(3) sostituito con model???
    save_model, model_to_remove = shouldSave(perplexityToSave, previousAccuracies)
    if save_model then
      local filename = paths.concat('logsGru', 'model.net' .. perplexityToSave)
      print('==> saving model to '..filename)
      torch.save(filename, model)
      local fileToRemove = paths.concat('logsGru', 'model.net' .. model_to_remove)
